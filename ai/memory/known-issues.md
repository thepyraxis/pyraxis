...
- **Resolved**: `three` added to `package.json`. Future-Proof Systems now uses a real WebGL globe (`NextStepGlobe.tsx`), same pattern as the Why-PYRAXIS globe. Shared `ParticleEngine.tsx` remains Canvas2D for every other section.
