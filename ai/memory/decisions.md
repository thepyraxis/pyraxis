...
## D-015 — Lenis smooth scroll adopted as the app's scroll driver

**Date:** Session (post-D-014, arbitrary-spacing/hero-frame-drop cleanup sessions).
**Decision:**
1. Added `lenis` (^1.3.26) as a direct dependency and built `providers/LenisProvider.tsx` — a single global instance, mounted in `GlobalProviders.tsx` between `MouseProvider` and `ScrollProvider` (outermost of the scroll-related providers, since `ScrollProvider` now reads from it).
2. Lenis's `raf` is driven by `gsap.ticker` (not its own independent `requestAnimationFrame` loop), and Lenis's `scroll` event calls `ScrollTrigger.update()` directly — one animation loop drives scroll, Lenis smoothing, and all ScrollTrigger scrub/pin work, per `ai/rules/architecture.md` #2 (never build systems outside the global GSAP loop).
3. `providers/ScrollProvider.tsx` internals were swapped from a `window.scrollY` + passive-listener + manual RAF-batch implementation to reading Lenis's own `scroll`/`velocity`/`progress` payload directly (Lenis already computes these; a second independent listener would have duplicated that work). The public `ScrollState` shape and `useScrollStore()` API are byte-for-byte unchanged — zero call sites required edits (there are currently no consumers of `useScrollStore` yet, confirmed by grep, so this is a zero-risk swap by construction).
4. `prefers-reduced-motion` handled inside `LenisProvider` itself (near-instant duration, no separate opt-out path), per `ai/rules/animation.md` #10 — never fully disable, keep it feeling native instead.
5. Deliberately did NOT adopt `@studio-freight/react-lenis` (or any Lenis React wrapper) — plain `lenis` package with a hand-rolled provider, to match this codebase's existing provider pattern (`ThemeProvider`, `MouseProvider`, etc. are all hand-rolled, not third-party context wrappers).
**Reasoning:** Native scroll had no momentum/easing control, which read as inconsistent against the site's motion language (`ai/rules/animation.md` #2: every animated object needs believable mass/momentum/inertia — this extended to the scroll input itself, not just section animations). `ai/specs/architecture/responsive-layout-system.md`'s non-goals section had already named Lenis as an anticipated future system, so this wasn't a speculative addition.
**Verification:** `tsc --noEmit`, `eslint .`, `next build` all clean; all 7 routes still generate; grep-confirmed zero `useScrollStore` consumers needed touching.
**Source:** User request (session) — spec written and locked first per CLAUDE.md spec-first discipline (`ai/specs/architecture/lenis-smooth-scroll.md`), then implemented.

## D-016 — Section-boundary particle handoff: additive instructions, not modulated existing ones

**Date:** Session (immediately following D-015, Lenis integration).
**Decision:**
1. Built `hooks/useSectionHandoff(sourceId, sectionRef, edge, options)` — the first real consumer of `providers/ParticleProvider.tsx`'s `useParticles()` (grep-confirmed zero prior usages; the provider was mounted at the app root but architecturally dormant). Uses `useScrollStore()` (Lenis-backed, D-015) to measure a section's own edge distance from the viewport edge and ramps a `density` value (small peak, `0.04` default share of the device budget) up/down as that edge crosses near the viewport boundary, sending `phase: "exiting"` (leaving section) or `phase: "entering"` (arriving section) under a dedicated `sourceId` (e.g. `handoff-hero-bottom`).
2. **Course-corrected mid-session**: the spec's first draft assumed sections already routed their primary particle system through the shared engine and that this hook would only need to modulate existing instructions' `phase`/`density`. Verified against live code (grep for `useParticles` usage, inspected `HeroAmbientParticles.tsx`/`ProblemAmbientParticles.tsx`/`GrowthEngineIconCanvas.tsx`/`GlobeCanvas.tsx`) that every section actually uses its own self-contained local Canvas2D/WebGL canvas instead — matches the already-tracked "particle engine ownership split" known-issue, not a bug. Corrected the spec before writing code: the hook sends a *new, separate, additive* low-density instruction per section edge, leaving every section's existing local canvas completely untouched.
3. Wired into all 10 applicable sections (`Hero`, `Problem`, `GrowthSystem`, `GrowthEngines`, `WhyPyraxis`, `Portfolio`, `FounderStory`, `Process`, `FutureProofSystems`, `CTA`, `Footer`) covering all 11 boundaries in `app/page.tsx`. `MarqueeTicker` excluded (pure CSS, no particle system involved on either side of it). `Footer` gets entering-only (terminal section, no next-section exit). 3 sections (`GrowthSystem`, `Portfolio`, `FutureProofSystems`) had no `sectionRef` before this session and needed one added, forwarded through the existing `Section` wrapper (`forwardRef`-based, already supported this).
4. Deliberately did NOT touch `Portfolio`'s or `FounderStory`'s explicit "no ambient motif"/"no particles" interior design — the handoff instruction lives only at the section's edge/seam, not inside its body, so it doesn't reintroduce the ambient effect those sections intentionally omit.
5. `prefers-reduced-motion`: the hook sends no instruction at all under reduced motion (checked via `usePrefersReducedMotion()`), rather than sending one and suppressing its visual effect — simpler, and there's no correctness reason (unlike a section's real content) to send a reduced-strength version.
6. **Flagged, not resolved**: `hooks/useEdgeFadeOpacity.ts` (pre-existing, powers the Hero→Problem boundary via opacity crossfade) directly violates `ai/rules/animation.md` #3 ("never use opacity crossfades"). This session's handoff instructions are layered in ADDITION to it at that one boundary, not a replacement — logged to `ai/memory/known-issues.md`, left as a separate decision for a future session (removing/replacing it risks visual regression on the one boundary that already has *something*, and wasn't asked for this session).
**Reasoning:** A single shared motif across 11 visually heterogeneous sections needed one substrate every section could plug into without migrating its own already-shipped visual system — the existing (but unused) global particle engine was that substrate. Additive-not-modulated keeps the change low-risk: every section's existing, already-verified local canvas is provably unaffected (no lines in it were touched).
**Verification:** `tsc --noEmit`, `eslint .`, `next build` all clean across all 10 modified section files + the new hook; `validate-all.mjs` clean after indexing the new spec file.
**Source:** User request (session) — spec written and locked first (`ai/specs/architecture/section-boundary-handoff.md`), corrected mid-session against live-code findings, then implemented.
