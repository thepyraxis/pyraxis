...
type WhyPyraxisFieldCanvasProps = {
...
/**
 * Ambient WebGL topo field for the Why-PYRAXIS ("How We Build") background.
 * Ported from the standalone NexusNode reference: a simplex-noise field
 * distorted by a pointer-following lens, with click-spawned ripples that
 * age out of a fixed 5-slot pool (no per-click allocation, no GC churn).
 * Re-tinted from the reference's neutral white to the brand purple
 * (--color-purple-500, #8b5cf6) so it reads as PYRAXIS rather than a copy.
 *
 ...
 * render nothing — Section already carries a plain dark background, so
 * "no field" degrades gracefully rather than needing a static fallback.
 */
export default function WhyPyraxisFieldCanvas({ sectionRef, className }: WhyPyraxisFieldCanvasProps) {
  ...
  useEffect(() => {
    ...
    // Brand purple (--color-purple-500, #8b5cf6), not the reference's white.
    ctx.uniform3f(uColor, 0x8b / 255, 0x5c / 255, 0xf6 / 255);
