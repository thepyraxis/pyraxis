...
type FutureProofFieldProps = {
  sectionRef: React.RefObject<HTMLElement | null>;
  className?: string;
};

export default function FutureProofField({ sectionRef, className }: FutureProofFieldProps) {
  ...
  useEffect(() => {
    ...
    function compile(type: number, src: string): WebGLShader | null {
      ...
      if (!ctx.getShaderParameter(s, ctx.COMPILE_STATUS)) {
        console.warn("[future-proof field shader]", ctx.getShaderInfoLog(s));
        return null;
      }
      return s;
    }
    ...
    if (!ctx.getProgramParameter(program, ctx.LINK_STATUS)) {
      console.warn("[future-proof field link]", ctx.getProgramInfoLog(program));
      return;
    }
